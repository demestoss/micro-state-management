import React, {
    createContext, FC, ReactNode, useContext, useEffect, useRef, useState,
} from 'react';
...
function getStore<T>(initialState: T): Store<T> {
...
function createStore<T>(initialState: T) {
    ...
    return <S extends unknown = T>(selector: (state: T) => S = (state) =>
        (state as unknown as S)) => useStore(store, selector);
}

function createStoreContext<T>(defaultValue?: T) {
    const StoreContext = createContext<Store<T> | undefined>(defaultValue && getStore(defaultValue));

    const Provider: FC<{ initialState: T; children: ReactNode }> = ({ initialState, children }) => {
        const valueRef = useRef<Store<T>>();
        if (!valueRef.current) {
            valueRef.current = getStore(initialState);
        }
    ...
    function useStoreMutation<S = T>(mutator: (mutate: (state: NextState<T>) => void) => void) {
        const store = useStoreContext();
        return () => mutator(store.set);
    }

    return [Provider, useStoreSelector, useStoreMutation] as const;
}

export { createStoreContext, getStore, useStore, createStore };
