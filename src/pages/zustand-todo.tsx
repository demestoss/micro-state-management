...
const createStore = () => create<StoreState, [['zustand/persist', StoreState], ["zustand/immer", never]]>(persist(immer((set) => ({
    todos: [],
...
const TodoItem: FC<{ id: string; title: string; done: boolean }> = ({ id, done, title }) => {
    ...
    return (
        <div className={cls(
            "flex justify-between rounded-md p-3 ",
            done ? "shadow-md" : 'shadow-lg'
        )}>
    ...
    return (
        <div ref={useRenderCount('NewTodo')}>
            <div className="flex flex-row space-x-1">
                ...
                />
                <button className="basis-1/4 bg-amber-200 rounded-md p-2 text-amber-800 hover:bg-amber-300" onClick={onClick}>
                    Add
                </button>
            </div>
        </div>
...
const ZustandTodo: FC = () => {
    return <div className="flex flex-col space-y-8 w-9/12 p-5 mx-auto">
...
function useRenderCount<T extends HTMLElement>(name?: string): React.Ref<T> {
    ...
    useEffect(() => {
        ...
        const node = Array.from(ref.current.children).find(node => node.classList.contains('render-count'));
